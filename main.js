(function () {
  ...
  function mailUrls(subject, body) {
    ...
    const gmail = `https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=${encodeURIComponent(CONTACT_EMAIL)}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    return { mailto, gmail };
  }

  function openCompose(subject, body) {
    const { gmail } = mailUrls(subject, body);
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        if (window.ZMDB) await window.ZMDB.saveMessage({ name, email, message, topic });
